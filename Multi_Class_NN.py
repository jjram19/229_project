...
num_classes = 3 # all genres


# Data formatting
y_data_total = np.genfromtxt('y_data_three_class.csv', delimiter = ',')
x_data_total = np.zeros((y_data_total.shape[0],num_input))
line_num = 0
with open('fft_data_three_class.csv', 'r') as _filehandler:
...
x_dev = x_data_total[:dev_size]; x_data = x_data_total[test_size:]
y_dev = y_data_total[:dev_size]; y_data = y_data_total[test_size:]
...
with tf.Session() as sess:
	...
	print("Test Accuracy", accuracy.eval({X: x_train, Y: t_train}))
	print("Accuracy:", accuracy.eval({X: x_dev, Y: y_dev}))
	print(epoch_cost)

	save_path = saver.save(sess, "./output/trained_multi.ckpt")
